...
void my_creatdir(myfs_t* myfs, int cur_dir_inode_number, const char* new_dirname) {
  ...
  //inode bitmap
  void* imapCopyPtr = malloc(BLKSIZE);
  memcpy(imapCopyPtr, (void*)&myfs->imap, BLKSIZE);
  block_t* imapCopy = (block_t*)imapCopyPtr;
  int newInodeNumber = 0, imapInd = 0;
  while(imapInd < BLKSIZE && newInodeNumber == 0) {
    if((imapCopy->data[imapInd/8] & 0x1 << (imapInd % 8)) == 0) {
      newInodeNumber = imapInd;
      imapCopy->data[imapInd/8] |= 0x1 << (imapInd % 8);
    }
    else {
      imapInd++;
    }
  ...
  if(newInodeNumber != 0) {
    memcpy((void*)&myfs->imap, imapCopyPtr, BLKSIZE);
    free(imapCopyPtr);
  }
  else {
  ...
  //block bitmap
  void* bmapCopyPtr = malloc(BLKSIZE);
  memcpy(bmapCopyPtr, (void*)&myfs->bmap, BLKSIZE);
  block_t* bmapCopy = (block_t*)bmapCopyPtr;
  int newBlock = 0, blockInd = 0;
  while(blockInd < BLKSIZE && newBlock == 0) {
    if((bmapCopy->data[blockInd/8] & 0x1 << (blockInd % 8)) == 0) {
      newBlock = blockInd;
      bmapCopy->data[blockInd/8] |= 0x1 << (blockInd % 8);
    }
    else {
      blockInd++;
    }
  ...
  if(newBlock != 0) {
    memcpy((void*)&myfs->bmap, bmapCopyPtr, BLKSIZE);
    free(bmapCopyPtr);
  }
  else {
    printf("Could not find free data block.\n");
    free(bmapCopyPtr);
    return;
  }

  //inode table
  void* inodeTableCopyPtr = malloc(BLKSIZE);
  memcpy(inodeTableCopyPtr, (void*)myfs->groupdescriptor.groupdescriptor_info.inode_table, BLKSIZE);
  inode_t* inodeTableCopy = (inode_t*)inodeTableCopyPtr;
  int parentDirPrevSize = inodeTableCopy[cur_dir_inode_number].size / sizeof(dirent_t); //store previous size of parent dir to use when updating dir entry
  inodeTableCopy[cur_dir_inode_number].size += sizeof(dirent_t);  //increase parent directory size by 1

  inodeTableCopy[newInodeNumber].size = 2 * sizeof(dirent_t);
  inodeTableCopy[newInodeNumber].blocks = 1;
  for(uint i = 1; i < 15; ++i) {
    inodeTableCopy[newInodeNumber].data[i] = NULL;
  }

  inodeTableCopy[newInodeNumber].data[0] = &(myfs->groupdescriptor.groupdescriptor_info.block_data[newBlock]);

  memcpy((void*)myfs->groupdescriptor.groupdescriptor_info.inode_table, inodeTableCopyPtr, BLKSIZE);
  free(inodeTableCopyPtr);

  //parent directory
  void* parentDataPtr = malloc(BLKSIZE);
  memcpy(parentDataPtr, (void*)myfs->groupdescriptor.groupdescriptor_info.inode_table[cur_dir_inode_number].data[0], BLKSIZE);
  dirent_t* parentData = (dirent_t*)parentDataPtr;
  dirent_t* parentDirent = &parentData[parentDirPrevSize];
  

  parentDirent->file_type = 2;
  parentDirent->inode = newInodeNumber;
  strcpy(parentDirent->name, new_dirname);
  parentDirent->name_len = strlen(new_dirname);

  memcpy((void*)myfs->groupdescriptor.groupdescriptor_info.inode_table[cur_dir_inode_number].data[0], parentDataPtr, BLKSIZE);
  free(parentDataPtr);
  
  //new directory
  void* newDataPtr = malloc(BLKSIZE);
  memcpy(newDataPtr, (void*)myfs->groupdescriptor.groupdescriptor_info.inode_table[newInodeNumber].data[0], BLKSIZE);
  dirent_t* newData = (dirent_t*)newDataPtr;

  dirent_t* newDirentSelf = &newData[0];
  newDirentSelf->file_type = 2;
  newDirentSelf->inode = newInodeNumber;
  strcpy(newDirentSelf->name, ".");
  newDirentSelf->name_len = 1;

  dirent_t* newDirentParent = &newData[1];
  newDirentParent->file_type = 2;
  newDirentParent->inode = cur_dir_inode_number;
  strcpy(newDirentParent->name, "..");
  newDirentParent->name_len = 2;

  memcpy((void*)myfs->groupdescriptor.groupdescriptor_info.inode_table[newInodeNumber].data[0], newDataPtr, BLKSIZE);
  free(newDataPtr);
